...
package cielsachen.javajeeps.mco2.service;
...
public class StorageBinService {
    ...
    public Map<Ingredient, Double> getIngredientCapacities() {
        Map<Ingredient, Double> ingredientToCapacity = new HashMap<>();

        for (Ingredient ingredient : Ingredient.values()) {
            ingredientToCapacity.put(ingredient,
                    getStorageBinsByIngredient(ingredient).stream().mapToDouble((sb) -> sb.getCapacity()).sum());
        }

        return ingredientToCapacity;
    }

/*
 * Simple Translator is simple CLI-based translator.
 ...
 * @brief The source file for the functions behind the Data Management menu and related utility
    ...
    for (entry_index = 0; entry_index < entries->length; entry_index++)
    {
        ...
        while (!is_containing_translation && translation_index < entry->translations_amount)
        {
            ...
            if ((strlen(translation->language) == 0 &&
                ...
                (strlen(translation->language) > 0 &&
                 strcmp(entry_translation->language, translation->language) == 0 &&
                 strcmp(entry_translation->word, translation->word) == 0))
            {
 ...
 */
int
get_translation_index(String20 language, Entry *entry)
{
    ...
    {
        if (strcmp(entry->translations[index].language, language) == 0)
        {
...
sort_translations(EntryList *entries)
{
    ...
    for (entry_index = 0; entry_index < entries->length; entry_index++)
    {
        ...
        {
            for (translation_index = 0; translation_index < entry->translations_amount - 1;
                 translation_index++)
            {
                for (other_translation_index = translation_index + 1;
        ...
        while (!is_exiting)
        {
            ...
            if (is_adding)
            {
                do
                {
                    ...
                    printf("Add another translation? " PRINT_COLOR_YELLOW
                           "([y]es / [ANY] no)" PRINT_COLOR_RESET ": ");
                    ...
                    if (is_adding_more_translations)
                    {
                        ...
                        printf(
                            "Input the language of the new translation in the entry " PRINT_COLOR_YELLOW
                            "(maximum of 20 characters)" PRINT_COLOR_RESET ": ");
                        ...
                        strcpy(entry.translations[entry.translations_amount].word,
                               translation.word);
                        ...
                        printf(PRINT_COLOR_GREEN
                               "Added the translation into the entry!" PRINT_COLOR_RESET "\n");
                    }
            ...
            if (entries->length < MAXIMUM_ENTRIES_LENGTH)
            {
                printf("Add another entry? " PRINT_COLOR_YELLOW
                       "([y]es / [ANY] no)" PRINT_COLOR_RESET ": ");
...
add_translations(EntryList *entries)
{
    ...
    while (!is_exiting)
    {
        if (is_different_entry)
        {
            ...
            printf("Input a translation in the entry " PRINT_COLOR_YELLOW
                   "(maximum of 20 characters)" PRINT_COLOR_RESET ": ");
        ...
        if ((entries_amount = has_entry(&translation, entries)))
        {
            if (entries_amount > 1)
            {
                do
                {
                    ...
                    do
                    {
                        ...
                        if (strlen(chosen_action_id) == 1 && toupper(chosen_action_id[0]) == 'P' &&
                            entry_count > 1)
                        {
                            is_valid_action_id = true;
                            entry_count--;
                        ...
                        else if (strlen(chosen_action_id) == 1 &&
                                 toupper(chosen_action_id[0]) == 'N' &&
                        ...
                        else if (sscanf(chosen_action_id, "%d", &chosen_entry_id) == 1 &&
                                 chosen_entry_id == entry_index)
                        {
                            is_valid_action_id = true;
            ...
            if (entry->translations_amount < MAXIMUM_ENTRY_TRANSLATIONS)
            {
                ...
                if (entry->translations_amount < MAXIMUM_ENTRY_TRANSLATIONS)
                {
                    ...
                    if (is_adding_more)
                    {
                        printf("Add to the same entry? " PRINT_COLOR_YELLOW
                               "([y]es / [ANY] no)" PRINT_COLOR_RESET ": ");
        ...
        if (is_deleting)
        {
            ...
            do
            {
                ...
                do
                {
                    ...
                    else if (sscanf(chosen_action_id, "%d", &chosen_translation_id) == 1 &&
                             chosen_translation_id > 0 &&
                ...
                if (is_deleting)
                {
                    if (entry->translations_amount > 1)
                    {
                        ...
                        printf(PRINT_COLOR_RED
                               "Deleted the translation from the entry!" PRINT_COLOR_RESET "\n");
                    ...
                    else
                    {
                        ...
                        printf(PRINT_COLOR_RED
                               "Deleted the entry from the stored list!" PRINT_COLOR_RESET "\n");
...
search_word(EntryList *entries)
{
    ...
    if (entries->length > 0)
    {
        printf("Input the word to search entries for " PRINT_COLOR_YELLOW
               "(maximum of 20 characters)" PRINT_COLOR_RESET ": ");
        ...
        if (output_file != NULL)
        {
            for (entry_index = 0; entry_index < entries->length; entry_index++)
            {
                for (translation_index = 0;
                     translation_index < entries->values[entry_index].translations_amount;
                     translation_index++)
                {
    ...
    if (input_file != NULL)
    {
        do
        {
            ...
            while (!(is_finished_reading =
                         fgets(input_buffer, sizeof input_buffer, input_file) == NULL) &&
                   strcmp(input_buffer, "\n") != 0 && strcmp(input_buffer, "\r\n") != 0)
            {
            ...
            if (!is_finished_reading)
            {
                if (!is_originally_empty_list)
                {
                    ...
                    printf("Add this entry? " PRINT_COLOR_YELLOW
                           "([Y]es / [ANY] no)" PRINT_COLOR_RESET ": ");
                ...
                if (should_add_entry)
                {
                    ...
                    if (!is_originally_empty_list)
                    {
                        ...
                        printf(PRINT_COLOR_GREEN
                               "Added the entry into the stored list!" PRINT_COLOR_RESET "\n");
                    }
                }
            }
        } while (!is_finished_reading);

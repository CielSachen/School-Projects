 ...
 * @param[in,out] sentence The sentence to translate.
 ...
 */
void
translate_sentence(String150 sentence, String20 input_language, String150 translated_sentence,
                   String20 output_language, EntryList *translation_entries)
{
    ...
    for (word_index = 0; word_index < words_amount; word_index++)
    {
        ...
        if ((entries_amount = has_entry(&temporary, translation_entries)))
        {
            ...
            do
            {
                translation_entry =
                    &translation_entries
                         ->values[get_entry_index(&temporary, translation_entries, entry_count++)];
            ...
            if (translation_index >= 0)
            {
                strcat(translated_sentence,
                       translation_entry->translations[translation_index].word);
            }
            else
            {
...
translate_input(EntryList *translation_entries)
{
    ...
    printf("Input the language to translate from " PRINT_COLOR_YELLOW
           "(maximum of 20 characters)" PRINT_COLOR_RESET ": ");

    get_string_input(input_language, MAXIMUM_LANGUAGE_LENGTH);

    printf("Input the language to translate into " PRINT_COLOR_YELLOW
           "(maximum of 20 characters)" PRINT_COLOR_RESET ": ");
    ...
    do
    {
        printf("Input the sentence to translate " PRINT_COLOR_YELLOW
               "(maximum of 150 characters)" PRINT_COLOR_RESET ": ");
        ...
        translate_sentence(input_sentence, input_language, translated_sentence, output_language,
                           translation_entries);
        ...
        printf("Translate another sentence? " PRINT_COLOR_YELLOW
...
/**
 * @brief Translates all sentences in a text file using the imported translation entries and exports
 * them into a new or overwritten text file.
 * @param[in] translation_entries The imported translation entries to use.
...
translate_text_file(EntryList *translation_entries)
{
    ...
    if (input_file != NULL)
    {
        while ((character = fgetc(input_file)) != EOF && sentences_amount < MAXIMUM_SENTENCES)
        {
            ...
            {
                if (character_index > 0 || (character_index == 0 && character != ' ' &&
                                            character != '\n' && character != '\r'))
                {
        ...
        for (sentence_index = 0; sentence_index < sentences_amount; sentence_index++)
        {
            ...
            translate_sentence(temporary, input_language, sentences[sentence_index],
                               output_language, translation_entries);
        }
        ...
        if (output_file != NULL)
        {
            ...
            printf(PRINT_COLOR_GREEN
                   "Exported the translated sentences into the file `%s`!" PRINT_COLOR_RESET "\n",
                   filename);
        }
        else
        {

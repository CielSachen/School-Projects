...
#include "simple_translator/translation.h"
...
translate_text_file(EntryList *translation_entries)
{
    ...
    if (input_file != NULL)
    {
        while ((character = fgetc(input_file)) != EOF && sentences_amount < MAXIMUM_SENTENCES)
        {
            if (character != '.' && character != '?' && character != '!')
            {
                if (character_index > 0 ||
                    (character_index == 0 && character != ' ' && character != '\n' && character != '\r'))
                {
        ...
        for (sentence_index = 0; sentence_index < sentences_amount; sentence_index++)
        {
            ...
            translate_sentence(temporary, input_language, sentences[sentence_index], output_language,
                               translation_entries);
        }

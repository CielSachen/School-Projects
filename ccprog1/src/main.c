...
int main(void) {
    ...
    printf(
        "Tides of Manila: A Merchant's Quest  Copyright (C) 2024  Louis Raphael V. Panaligan\n"
    ...
    while (nPlayerTurns < MAXIMUM_PLAYER_TURNS && !bIsQuitting) {
        ...
        if (cCurrentPortId == 'T') {
            nCoconutMarketPrice =
                generateRandomPrice(TONDO_MINIMUM_COCONUT_PRICE, TONDO_MAXIMUM_COCONUT_PRICE);
            nRiceMarketPrice =
                generateRandomPrice(TONDO_MINIMUM_RICE_PRICE, TONDO_MAXIMUM_RICE_PRICE);
            nSilkMarketPrice =
                generateRandomPrice(TONDO_MINIMUM_SILK_PRICE, TONDO_MAXIMUM_SILK_PRICE);
            nGunMarketPrice = generateRandomPrice(TONDO_MINIMUM_GUN_PRICE, TONDO_MAXIMUM_GUN_PRICE);
        } else if (cCurrentPortId == 'M') {
            nCoconutMarketPrice =
                generateRandomPrice(MANILA_MINIMUM_COCONUT_PRICE, MANILA_MAXIMUM_COCONUT_PRICE);
            nRiceMarketPrice =
                generateRandomPrice(MANILA_MINIMUM_RICE_PRICE, MANILA_MAXIMUM_RICE_PRICE);
            nSilkMarketPrice =
                generateRandomPrice(MANILA_MINIMUM_SILK_PRICE, MANILA_MAXIMUM_SILK_PRICE);
            nGunMarketPrice =
                generateRandomPrice(MANILA_MINIMUM_GUN_PRICE, MANILA_MAXIMUM_GUN_PRICE);
        } else if (cCurrentPortId == 'P') {
            nCoconutMarketPrice =
                generateRandomPrice(PANDAKAN_MINIMUM_COCONUT_PRICE, PANDAKAN_MAXIMUM_COCONUT_PRICE);
            nRiceMarketPrice =
                generateRandomPrice(PANDAKAN_MINIMUM_RICE_PRICE, PANDAKAN_MAXIMUM_RICE_PRICE);
            nSilkMarketPrice =
                generateRandomPrice(PANDAKAN_MINIMUM_SILK_PRICE, PANDAKAN_MAXIMUM_SILK_PRICE);
            nGunMarketPrice =
                generateRandomPrice(PANDAKAN_MINIMUM_GUN_PRICE, PANDAKAN_MAXIMUM_GUN_PRICE);
        } else {
            nCoconutMarketPrice =
                generateRandomPrice(SAPA_MINIMUM_COCONUT_PRICE, SAPA_MAXIMUM_COCONUT_PRICE);
            nRiceMarketPrice =
                generateRandomPrice(SAPA_MINIMUM_RICE_PRICE, SAPA_MAXIMUM_RICE_PRICE);
            nSilkMarketPrice =
                generateRandomPrice(SAPA_MINIMUM_SILK_PRICE, SAPA_MAXIMUM_SILK_PRICE);
        ...
        while (!bIsNavigating && !bIsQuitting) {
            nPlayerProfit =
                ((nPlayerBalance - nPlayerInitialBalance) / (nPlayerInitialBalance * 1.0)) * 100;

            int nShipTotalCargoAmount = nShipCoconutCargoAmount + nShipRiceCargoAmount +
                                        nShipSilkCargoAmount + nShipGunCargoAmount;

            printMainScreen(nPlayerTurns, cCurrentPortId, nPlayerMerchantId, nPlayerBalance,
                            nPlayerProfit, nPlayerProfitTarget, nShipTotalCargoAmount,
                            nShipCargoLimit, nShipCoconutCargoAmount, nShipRiceCargoAmount,
                            nShipSilkCargoAmount, nShipGunCargoAmount, nCoconutMarketPrice,
                            nRiceMarketPrice, nSilkMarketPrice, nGunMarketPrice);

            char cChosenActionId = promptPlayerForActionId(
                nShipTotalCargoAmount, nShipCargoLimit, nPlayerBalance, nCoconutMarketPrice,
                nRiceMarketPrice, nSilkMarketPrice, nGunMarketPrice);
            ...
            if (cChosenActionId == 'B' || cChosenActionId == 'S') {
                char cChosenCargoId = promptPlayerForCargoId(
                    cChosenActionId, nPlayerBalance, nShipCoconutCargoAmount, nRiceMarketPrice,
                    nSilkMarketPrice, nGunMarketPrice, nShipCoconutCargoAmount,
                ...
                if (cChosenCargoId != 'X') {
                    ...
                    int nCargoAmount = promptPlayerForCargoAmount(
                        cChosenActionId, nShipCargoLimit - nShipTotalCargoAmount, cChosenCargoId,
                        nPlayerBalance, nCoconutMarketPrice, nRiceMarketPrice, nSilkMarketPrice,
                        nGunMarketPrice, nShipCoconutCargoAmount, nShipRiceCargoAmount,
                        nShipSilkCargoAmount, nShipGunCargoAmount);
                    ...
                    if (bIsConfirmed) {
                        ...
                        tradeCargo(cChosenActionId, &nPlayerBalance, nCargoPrice, nCargoAmount,
                                   nShipCargoAmount);

                        printTransactionScreen(cChosenActionId, cChosenCargoId, nCargoAmount,
                                               nCargoPrice);
                ...
                if (bIsUpgrading) {
                    ...
                    upgradeShip(&nPlayerBalance, nShipUpgradeCost, &nShipCargoLimit,
                                nShipNewCargoLimit);
            ...
            if (nStormTurnDuration > 0) {
                ...
                if (nStormTurnDuration == 0) {
                    ...
                    if (bAdjustStormStrength) {
                        ...
                        setStormStrength(&nStormWindSignalNumber, nStormNewWindSignalNumber,
                                         &nPostponedDepartureChance);
                    }
                }

                printWeatherReportScreen(bIsNewStorm, nStormOldWindSignalNumber,
                                         nStormWindSignalNumber, nPostponedDepartureChance,
                                         nStormTurnDuration);
    ...
    printGameEndScreen(nPlayerProfit, nPlayerProfitTarget, nPlayerBalance, nPlayerInitialBalance,
                       nPlayerTurns);

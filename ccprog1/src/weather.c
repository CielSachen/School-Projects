 ...
 * @pre @p nTruePercentChance must have a positive integer value greater than or equal to  `1` and
...
/**
 * @brief Generates a random duration in turns for a storm within the passed minimum and constant
...
int generateRandomStormTurnDuration(int nMinimumTurnDuration) {
    return (rand() % (MAXIMUM_STORM_TURN_DURATION - nMinimumTurnDuration + 1)) +
           nMinimumTurnDuration;
}

/**
 * @brief Sets the wind signal number of the storm to the given new wind signal number and sets its
 * debuffs.
 ...
 * @param[out] nPostponedDepartureChance The percentage chance of the player being unable to depart
 * their current port.
 */

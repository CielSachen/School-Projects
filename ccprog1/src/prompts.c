...
/**
 * @brief Prompts the player to input their preferred merchant ID.
 * @returns An integer value within the range of `0` and `999`.
 * @bug Inputting a character or a string, or even just a long integer or float, as a response will
 * result in an infinite loop because the buffer cannot be cleared with any of the prescribed
 * functions.
 */
int promptPlayerForMerchantId(void) {
    ...
    do {
        ...
        if (nGivenMerchantId < MINIMUM_MERCHANT_ID_VALUE ||
            nGivenMerchantId > MAXIMUM_MERCHANT_ID_VALUE) {
...
/**
 * @brief Prompts the player to input the number of gold coins they want to start with.
...
/**
 * @brief Prompts the player to input their desired profit target.
...
/**
 * @brief Prompts the player to input a confirmation of their prior decision.
...
/**
 * @brief Prompts the player to input the ID of the action they want to do.
 ...
 * @returns A character value of `B` for the buy screen, `S` for the sell screen, `U` for the ship
 * upgrade screen, `N` for the navigation screen, or `Q` for quitting the game.
 */
char promptPlayerForActionId(int nShipTotalCargoAmount, int nShipCargoLimit, int nPlayerBalance,
                             int nCoconutMarketPrice, int nRiceMarketPrice, int nSilkMarketPrice,
                             int nGunMarketPrice) {
    ...
    if (nShipTotalCargoAmount > 0) {
        printf("%*c‣ [S] Sell Cargo\n", nConsoleLeftPaddingSize, ' ');
    }
    ...
    do {
        ...
        if (cChosenActionId != BUY_ACTION_ID && cChosenActionId != SELL_ACTION_ID &&
            cChosenActionId != UPGRADE_ACTION_ID && cChosenActionId != NAVIGATE_ACTION_ID &&
            cChosenActionId != QUIT_ACTION_ID) {
            setConsoleColorToRed();
            printf("%*cPlease only enter one of the provided character options!\n",
                   nConsoleLeftPaddingSize, ' ');
            resetConsoleColor();
            ...
            resetConsoleColor();
                   ...
                   nShipCargoLimit == TIER_FOUR_SHIP_CARGO_LIMIT) {
            setConsoleColorToRed();
...
/**
 * @brief Prompts the player to input the ID of the cargo they want to trade.
 ...
 * @returns A character value of `C` for coconut, `R` for rice, `S` for silk, `G` for gun, or `X`
 * for returning to the main menu.
 */
char promptPlayerForCargoId(char cTransactionTypeId, int nPlayerBalance, int nCoconutMarketPrice,
                            int nRiceMarketPrice, int nSilkMarketPrice, int nGunMarketPrice,
                            int nShipCoconutCargoAmount, int nShipRiceCargoAmount,
    ...
    if (cTransactionTypeId == BUY_ACTION_ID) {
        if (nPlayerBalance > nCoconutMarketPrice) {
            printf("%*c‣ [C] Coconut\n", nConsoleLeftPaddingSize, ' ');
        }

        if (nPlayerBalance > nRiceMarketPrice) {
            printf("%*c‣ [R] Rice\n", nConsoleLeftPaddingSize, ' ');
        }

        if (nPlayerBalance > nSilkMarketPrice) {
            printf("%*c‣ [S] Silk\n", nConsoleLeftPaddingSize, ' ');
        }

        if (nPlayerBalance > nGunMarketPrice) {
            printf("%*c‣ [G] Gun\n", nConsoleLeftPaddingSize, ' ');
        }
    } else {
        if (nShipCoconutCargoAmount > 0) {
            printf("%*c‣ [C] Coconut\n", nConsoleLeftPaddingSize, ' ');
        }

        if (nShipRiceCargoAmount > 0) {
            printf("%*c‣ [R] Rice\n", nConsoleLeftPaddingSize, ' ');
        }

        if (nShipSilkCargoAmount > 0) {
            printf("%*c‣ [S] Silk\n", nConsoleLeftPaddingSize, ' ');
        }

        if (nShipGunCargoAmount > 0) {
            printf("%*c‣ [G] Gun\n", nConsoleLeftPaddingSize, ' ');
        }
    }
    ...
    do {
        ...
        if (cChosenCargoId != COCONUT_CARGO_ID && cChosenCargoId != RICE_CARGO_ID &&
            cChosenCargoId != SILK_CARGO_ID && cChosenCargoId != GUN_CARGO_ID &&
            cChosenCargoId != CANCEL_ACTION_ID) {
            setConsoleColorToRed();
            printf("%*cPlease only enter one of the provided character options!\n",
                   nConsoleLeftPaddingSize, ' ');
            resetConsoleColor();
            ...
            printf("%*cYou do not have enough gold coins to buy silk!\n", nConsoleLeftPaddingSize,
                   ' ');
            resetConsoleColor();
        } else if (cTransactionTypeId == BUY_ACTION_ID && cChosenCargoId == 'G' &&
                   nPlayerBalance < nGunMarketPrice) {
            setConsoleColorToRed();
            ...
            resetConsoleColor();
        } else if (cTransactionTypeId == SELL_ACTION_ID && cChosenCargoId == RICE_CARGO_ID &&
                   nShipRiceCargoAmount == 0) {
            setConsoleColorToRed();
            printf("%*cYou do not have any rice to sell!\n", nConsoleLeftPaddingSize, ' ');
            resetConsoleColor();
        } else if (cTransactionTypeId == SELL_ACTION_ID && cChosenCargoId == SILK_CARGO_ID &&
                   nShipSilkCargoAmount == 0) {
            setConsoleColorToRed();
            printf("%*cYou do not have any silk to sell!\n", nConsoleLeftPaddingSize, ' ');
            resetConsoleColor();
        } else if (cTransactionTypeId == SELL_ACTION_ID && cChosenCargoId == GUN_CARGO_ID &&
                   nShipGunCargoAmount == 0) {
            setConsoleColorToRed();
...
/**
 * @brief Prompts the player to input the amount of cargo they want to trade.
 * @param cTransactionTypeId The ID of the type of transaction to make.
 * @param nShipCargoLimit The maximum amount of cargo that the ship can carry.
 ...
 * functions.
 */
int promptPlayerForCargoAmount(char cTransactionTypeId, int nShipCargoLimit, char cCargoId,
                               int nPlayerBalance, int nCoconutMarketPrice, int nRiceMarketPrice,
                               int nSilkMarketPrice, int nGunMarketPrice,
                               int nShipCoconutCargoAmount, int nShipRiceCargoAmount,
                               int nShipSilkCargoAmount, int nShipGunCargoAmount) {
    int nConsoleLeftPaddingSize = 57;
...
/**
 * @brief Prompts the player to input a confirmation on whether they want to upgrade their ship or
 * not.
    ...
    do {
        ...
        if (cGivenBooleanId != YES_ID && cGivenBooleanId != NO_ID) {
            ...
            printf("%*cPlease only enter one of the provided character options!\n",
                   nConsoleLeftPaddingSize, ' ');
            resetConsoleColor();
        } else if ((nShipCurrentCargoLimit == TIER_ONE_SHIP_CARGO_LIMIT &&
                    nPlayerBalance < TIER_TWO_SHIP_UPGRADE_COST) ||
                   (nShipCurrentCargoLimit == TIER_TWO_SHIP_CARGO_LIMIT &&
                    nPlayerBalance < TIER_THREE_SHIP_UPGRADE_COST) ||
...
/**
 * @brief Prompts the player to input the ID of the port they want to navigate to.
 * @param cCurrentPortId The ID of the current port.
 * @pre @p cCurrentPortId must have a character value of `T`, `M`, `P`, or `S`.
 * @returns A character value of `T` for Tondo, `M` for Manila, `P` for Pandakan, `S` for Sapa, or
 * `X` for returning to the main menu.
 */
char promptPlayerForPortId(char cCurrentPortId) {
    ...
    do {
        ...
        if (cChosenPortId != TONDO_PORT_ID && cChosenPortId != MANILA_PORT_ID &&
            cChosenPortId != PANDAKAN_PORT_ID && cChosenPortId != SAPA_PORT_ID &&
            cChosenPortId != CANCEL_ACTION_ID) {
            setConsoleColorToRed();
            printf("%*cPlease only enter one of the provided character options!\n",
                   nConsoleLeftPaddingSize, ' ');
            resetConsoleColor();

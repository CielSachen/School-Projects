...
int main(void) {
    ...
    if (fgets(in_buff, sizeof in_buff, in_file) == NULL) {
        fclose(in_file);
    ...
    sscanf(in_buff, "%zu", &in_size);

    const size_t pts_mem_size = sizeof(Point) * in_size;
    ...
    for (size_t i = 0; i < in_size; i++) {
        ...
        if (sscanf(in_buff, " %lf %lf", &pt->x, &pt->y) != 2) {
            free(pts);
    ...
    if (!slow_scan(pts, in_size, &convex_pts, &convex_pnt_cnt)) {
    ...
    print_convex_points(out_file, convex_pts, convex_pnt_cnt);
